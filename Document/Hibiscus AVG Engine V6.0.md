...
可在事件`RE_Extension`中对替换规则进行扩展，如将`<Gold>`替换为当前持有的金币数等。若替换后的字符串需要包含特殊字符，则需要使用替换功能进行转义。例如，若替换的文本中含有默认的分隔符`:`，需要将其转义为`<%COL>`，并在显示前对显示文本进行还原。还原时，会按照优先级依次顺序进行替换。
...
非转义表达式在脚本更新阶段进行替换，`<>`>`<VID>`>`<SID>`

随后是对象信息，`<InternalID_Name>`>`<PFID>`>`<SCID>`
...
| --------------------------------- | ---------------- |
| `<LocalCode>`                     | 语言区域代码     |
| `(<LocalCode)(DisplayName_.*)(>)` | 语言显示名称     |
| `<Gold>`                          | 当前金币数       |
| `<Calender_Year>`                 | 游戏内日历年份   |
| `<Calender_Month>`                | 游戏内日历月份   |
| `<Calender_Date>`                 | 游戏内日历日期   |
| `<StaticVideo>`                   | 静态视频启用情况 |
| `<PlayTime>`                      | 游玩时间         |
| `<CurrentTime>`                   | 当前时间         |
...
| 名称           | 行为                     |
| -------------- | ------------------------ |
| `Erase At End` | 在程序退出后清除缓存     |
| `Erase At EOF` | 在脚本执行结束后清除缓存 |

若在脚本设置中指定了缓存行为，脚本设置会覆盖默认设置。如`#Settings=LoadAtStart|EraseAtEOF`会强制在读取脚本时加载所有图像素材，并在脚本执行结束后清除缓存
...
##### `@GraphicErase=MemLimit`

尝试清理未引用的图像缓存，直到当前内存占用低于`MemLimit`或无可清理缓存。`MemLimit = -1`将按照默认设置清理，`MemLimit = 0`将清理全部未引用缓存
...
负数图像ID为系统占用，通常情况下请勿在指令中引用。该系列ID可能会随着未来的更新而改变，如确认有需要，请使用`<InternalID_Name>`获取

| 类别         | 默认ID | 初始化                       | 替换表达式                 |
| ------------ | ------ | ---------------------------- | -------------------------- |
| 姓名栏       | -1     | namenull.png                 | `<InternalID_NameBack>`    |
| 对话框       | -2     | DiaNull.png / VNModeNull.png | `<InternalID_DiaBack>`     |
| 头像栏       | -3     | null.png                     | `<InternalID_AvatarBack>`  |
| \-\-\-       | \-\-\- | \-\-\-                       | \-\-\-                     |
| 底板         | -4     | null.png                     | `<InternalID_Bottom>`      |
| CG           | -5     | null.png                     | `<InternalID_CG>`          |
| 头像         | -6     | AvatarNull.png               | `<InternalID_Avatar>`      |
| \-\-\-       | \-\-\- | \-\-\-                       | \-\-\-                     |
| Sepia Toning | -100   | N/A                          | `<InternalID_SepiaToning>` |
| 景深         | -1000  | N/A                          | `<InternalID_Depth>`       |
| ...          | ...    | ...                          | ...                        |
| \-\-\-       | \-\-\- | \-\-\-                       | \-\-\-                     |
| 错误提示     | -65535 | N/A                          | `<InternalID_Error>`       |
