...
echo f | xcopy "%curPath%\savings\_Global\Data_Template" "%ContentPath%\savings\_Global\Data_Template" /s /e
...
del "%ContentPath%\data\dialogue\_Ref.asc" /f /s /q
...
@echo Login...
%SteamCMDPath% +login %SteamUser% %SteamPassword% %SteamGuard% 

@echo DRM...
%SteamCMDPath% +drm_wrap %APPID% "%curPath%\%AppName%.exe" "%ContentPath%\%AppName%.exe" drmtoolp 0
...
%SteamCMDPath% +run_app_build %ScriptPath% +quit
